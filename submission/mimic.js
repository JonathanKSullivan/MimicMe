...
function drawFeaturePoints(canvas, img, face) {
  ...
  for (var id in face.featurePoints) {
    ...
    ctx.beginPath();
    ctx.arc(featurePoint.x, featurePoint.y, 2, 0, 360, 0);
    ctx.fill();
    ctx.stroke();
...
function initGame() {
  ...
  start_time = 0
  time = 0
  right_audio = new Audio('right_answer.wav ');
  wrong_audio = new Audio('wrong_answer.wav');
}
...
function runGame(face) {
  if (targetEmoji == null) {
    targetEmoji = pickTargetEmoji(emojis);
  }
  if (isFaceMatched(face)){
    targetEmoji = pickTargetEmoji(emojis);
    didWinGame(true);
  }
}
